cd ${{ upper (snake name) }}_HOME/database/{{kebab name}}
...
--url=jdbc:postgresql://ENTER_HERE/{{kebab name}} \
--username {{kebab name}} --password {{kebab name}} \
--reference-url=jdbc:postgresql://127.0.0.1/{{kebab name}} \
--reference-username {{kebab name}} \
--reference-password {{kebab name}};
...
${CONTENT}
 ...
 (cat changelogs/$FILENAME | xml tr ${{ upper (snake name) }}_HOME/local-dev/scripts/utils/sort-changesets.xml | xml fo --nsclean) > tmp.xml 
